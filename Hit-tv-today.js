let sliderTvToday = document.getElementById('slidertvToday');
let innerSliderTvToday = '';
fetch(`https://api.themoviedb.org/3/trending/tv/day?api_key=c6b4ec789301ea172457fcb6ecc3f733`)
    .then((res) =>
        res.json()
    )
    .then((data) => {
        ...
        for (var x in data.results) {
            innerSliderTvToday += `<div class="card bg-transparent border-0 text-white m-2" data-bs-toggle="modal" data-bs-target="#exampleModal" id=${data.results[x].id}>
                                <img src="https://image.tmdb.org/t/p/w300${data.results[x].poster_path}" style="width:90%;" alt = "...">
                            </div>`;
        }
        sliderTvToday.innerHTML = `<div class="text-light">
                                    <h2 class="m-0 pt-2">อันดับรายการทีวีสูงสุดวันนี้</h2>
                                </div>
                                <div class="slider-tv-today m-0">
                                    ${innerSliderTvToday}
                                </div>`

        sliderTvToday.addEventListener('click', function (e) {
