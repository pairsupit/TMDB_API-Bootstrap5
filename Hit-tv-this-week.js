...
fetch(`https://api.themoviedb.org/3/trending/tv/week?api_key=c6b4ec789301ea172457fcb6ecc3f733`)
    ...
    .then((data) => {
        ...
        for (var x in data.results) {
            innerSlidertv += `<div class="card bg-transparent border-0 text-white m-2 hover" data-bs-toggle="modal" data-bs-target="#exampleModal" id=${data.results[x].id}>
        ...
        sliderTv.innerHTML = `<div class="text-light">
                                <h2 class="m-0 pt-2">อันดับรายการทีวีสูงสุดสัปดาห์นี้</h2>
                            </div>
                            <div class="slider-tv-this-week m-0">
                                ${innerSlidertv}
                            </div>`;

        sliderTv.addEventListener('click', function (e) {
