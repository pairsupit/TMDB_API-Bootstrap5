...
fetch(`https://api.themoviedb.org/3/trending/movie/week?api_key=c6b4ec789301ea172457fcb6ecc3f733`)
    ...
    .then((data) => {
        ...
        for (var x in data.results) {
            ...
            innerSliderMovie += `<div class="card bg-transparent border-0 text-white m-2" data-bs-toggle="modal" data-bs-target="#exampleModal" id=${data.results[x].id}>
        ...
        sliderMovie.innerHTML = `<div class="text-light">
                                    <h2 class="m-0 pt-2">อันดับหนังสูงสุดสัปดาห์นี้</h2>
                                </div>
                                <div class="slider-movie-this-week m-0">
                                    ${innerSliderMovie}
                                </div>`

        sliderMovie.addEventListener('click', function (e) {
            // console.log('click sliderMovie')
